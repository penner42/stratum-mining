...
class BlockUpdater(object):
    ...
    def schedule(self):
        ...
        log.debug("Merkle update in next %.03f sec" % \
                  ((Interfaces.template_registry.last_update + settings.MERKLE_REFRESH_INTERVAL)-Interfaces.timestamper.time()))
        self.clock = reactor.callLater(when, self.run)
        
    def _get_next_time(self):
        when = settings.PREVHASH_REFRESH_INTERVAL - (Interfaces.timestamper.time() - Interfaces.template_registry.last_update) % \
    ...
    def run(self):
        ...
        try:             
            if Interfaces.template_registry.last_block:
                current_prevhash = "%064x" % Interfaces.template_registry.last_block.hashPrevBlock
            else:
                current_prevhash = None
                
            log.info("Checking for new block.")
            prevhash = util.reverse_hash((yield Interfaces.template_registry.bitcoin_rpc.prevhash()))
            if prevhash and prevhash != current_prevhash:
                log.info("New block! Prevhash: %s" % prevhash)
                update = True
            
            elif Interfaces.timestamper.time() - Interfaces.template_registry.last_update >= settings.MERKLE_REFRESH_INTERVAL:
